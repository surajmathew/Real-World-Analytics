...
fit.OWA <- function(v_df,output.1="output1.txt",stats.1="stats1.txt") {
  # preliminary information										
  ycol <- ncol(v_df)
  n <- ycol-1
  instances <- nrow(v_df)
  ...
  all.const <- array(0,0)
  # reordered g(x_i)
  for(k in 1:instances) {const.i <- as.numeric(sort(v_df[k,1:n])) 
  all.const <- rbind(all.const,const.i)
  }
  # residual coefficients
  resid.pos <- -1*diag(instances)
  ...
  new.yvals <- array(0,instances)
  for(k in 1:instances) {
    new.yvals[k] <- OWA(v_df[k,1:n],t(w.weights))
  }
  # write the output						
  
  
  write.table(cbind(v_df,new.yvals),output.1,row.names = FALSE, col.names = FALSE)
  # write some stats	
  RMSE <- (sum((new.yvals - v_df[,ycol])^2)/instances)^0.5
  av.l1error <- sum(abs(new.yvals - v_df[,ycol]))/instances
  
  
  somestats <- rbind(c("RMSE",RMSE),c("Av. abs error",av.l1error),c("Pearson correlation",cor(v_df[,ycol],new.yvals)),c("Spearman correlation",cor(v_df[,ycol],new.yvals,method="spearman")),c("Orness",sum(w.weights*(1:n-1)/(n-1))), c("i","w_i "),cbind(1:n,w.weights))
